...
def _focus_browser_window() -> bool:
    """Focus browser window (Chrome, Edge, Firefox, Brave, Opera) if available."""
    if not is_windows() or not _PYAUTOGUI:
        return False
    try:
...
def _handle_pause(parameters: dict, player=None) -> str:
    """Pause playback (k or space or playpause)."""
...
def _handle_stop(parameters: dict, player=None) -> str:
    """Stop/pause playback."""
    _focus_browser_window()
    if _PYAUTOGUI:
        pyautogui.press("k")
...
def _handle_resume(parameters: dict, player=None) -> str:
    """Resume playback."""
    _focus_browser_window()
    if _PYAUTOGUI:
        pyautogui.press("k")
...
def _handle_next(parameters: dict, player=None) -> str:
    """Play next video/song (Shift+N)."""
    _focus_browser_window()
    if _PYAUTOGUI:
...
def _handle_prev(parameters: dict, player=None) -> str:
    """Play previous video/song (Shift+P)."""
    _focus_browser_window()
    if _PYAUTOGUI:
        pyautogui.hotkey("shift", "p")
        return "Playing previous video, sir."
    return "pyautogui is not installed, sir."


def _handle_seek_forward(parameters: dict, player=None) -> str:
    """Seek forward (l = 10s, right = 5s)."""
    _focus_browser_window()
    seconds = int(parameters.get("seconds") or 10)
    if _PYAUTOGUI:
        if seconds <= 5:
            ...
            for _ in range(presses):
                pyautogui.press("l")
                time.sleep(0.05)
        return f"Seeked forward {seconds} seconds, sir."
    return "pyautogui is not installed, sir."


def _handle_seek_backward(parameters: dict, player=None) -> str:
    """Seek backward (j = 10s, left = 5s)."""
    _focus_browser_window()
    seconds = int(parameters.get("seconds") or 10)
    if _PYAUTOGUI:
        if seconds <= 5:
            ...
            for _ in range(presses):
                pyautogui.press("j")
                time.sleep(0.05)
        return f"Seeked backward {seconds} seconds, sir."
    return "pyautogui is not installed, sir."


def _handle_seek(parameters: dict, player=None) -> str:
    """Seek to a specific percentage (0-90%) using number keys 0-9."""
    _focus_browser_window()
    percent = parameters.get("percent")
    if percent is not None:
        try:
            p = int(percent)
            digit = str(min(9, max(0, p // 10)))
            if _PYAUTOGUI:
                pyautogui.press(digit)
                return f"Seeked to {p}%, sir."
        except Exception:
            pass
    ...
    if seconds is not None:
        s = int(seconds)
        if s >= 0:
            return _handle_seek_forward({"seconds": s}, player)
        else:
            return _handle_seek_backward({"seconds": abs(s)}, player)

    if _PYAUTOGUI:
        pyautogui.press("0")
        return "Seeked to start of video, sir."
    return "pyautogui is not installed, sir."
...
_ACTION_MAP = {
    ...
    "toggle_pause":  _handle_pause,
    "next":          _handle_next,
    "next_song":     _handle_next,
    "next_video":    _handle_next,
    "prev":          _handle_prev,
    "previous":      _handle_prev,
    "prev_song":     _handle_prev,
    "prev_video":    _handle_prev,
    "seek_forward":  _handle_seek_forward,
    "forward":       _handle_seek_forward,
    "skip_forward":  _handle_seek_forward,
    "seek_backward": _handle_seek_backward,
    "backward":      _handle_seek_backward,
    "rewind":        _handle_seek_backward,
    "seek":          _handle_seek,
...
def youtube_video(
    ...
    if handler is None:
        avail = ", ".join(sorted(set(_ACTION_MAP.keys())))
        return (
