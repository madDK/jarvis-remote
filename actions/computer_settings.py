...
def volume_down():
    if _OS == "Windows":
        try:
            ...
            print(f"[Settings] volume_down failed: {e}")
...
def volume_mute():
    if _OS == "Windows":
        try:
            vol = _get_win_volume_endpoint()
            vol.SetMute(1 - vol.GetMute(), None)
        except Exception as e:
            print(f"[Settings] volume_mute failed: {e}")
...
def volume_set(value: int):
    ...
    if _OS == "Windows":
        try:
            ...
            print(f"[Settings] pycaw volume_set failed, using keypress fallback: {e}")
            pyautogui.press("volumemute")
            pyautogui.press("volumemute")
    elif _OS == "Darwin":
...
def brightness_set(value: int):
    value = max(0, min(100, int(value)))
    if _OS == "Darwin":
        # AppleScript keys emulating or settings command if tool installed
        pass
    elif _OS == "Linux":
        if subprocess.run(["which", "brightnessctl"],
                capture_output=True).returncode == 0:
        ...
        try:
            ...
            print(f"[Settings] brightness_set failed on Windows: {e}")
...
def lock_screen():
    if _OS == "Windows":
        try:
            ...
            print(f"[Settings] LockWorkStation failed, trying fallback: {e}")
...
def set_wifi_state(enable: bool):
    if _OS == "Darwin":
        ...
        state_cmd = "Enable-NetAdapter" if enable else "Disable-NetAdapter"
        try:
            subprocess.run(
                ["powershell", "-Command",
                 f"$adapter = Get-NetAdapter | Where-Object {{$_.PhysicalMediaType -eq 'Native 802.11'}};"
                 f"{state_cmd} -Name $adapter.Name -Confirm:$false"],
                capture_output=True, timeout=10, **_WIN_HIDE
            )
        except Exception as e:
            print(f"[Settings] set_wifi_state Windows failed: {e}")
    else:
        state = "on" if enable else "off"
        try:
            subprocess.run(["nmcli", "radio", "wifi", state], capture_output=True)
        except Exception as e:
            print(f"[Settings] set_wifi_state Linux failed: {e}")
...
- Return ONLY the JSON, no explanation, no markdown."""
    ...
    try:
        resp = _client.models.generate_content(model="gemini-2.5-pro", contents=prompt)
...
def computer_settings(
    ...
    if (not action or action not in ACTION_MAP) and (description or raw_action):
        desc = description or raw_action
        detected = _detect_action(desc)
        detected_action = detected.get("action", "").lower().strip().replace(" ", "_").replace("-", "_")
        if detected_action in ACTION_MAP or detected_action in ("volume_set", "brightness_set"):
    ...
    if action in ("volume_set", "set_volume", "change_volume", "volume"):
        try:
            val_str = re.sub(r"\D", "", str(value or "50"))
            val_int = int(val_str) if val_str else 50
            volume_set(val_int)
            return f"Volume set to {val_int}%."
        except Exception as e:
            return f"Could not set volume: {e}"

    if action in ("brightness_set", "set_brightness", "change_brightness", "brightness"):
        try:
            val_str = re.sub(r"\D", "", str(value or "50"))
            val_int = int(val_str) if val_str else 50
            brightness_set(val_int)
            return f"Brightness set to {val_int}%."
        except Exception as e:
