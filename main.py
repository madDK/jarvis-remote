...
if sys.platform == "win32":
    import io
    # Force UTF-8 encoding for stdout/stderr to prevent CP1252/charmap print crashes with emojis
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        "name": "youtube_video",
        "description": (
            "Controls YouTube and media playback. Use for: playing videos/music, "
            "pausing, stopping, resuming, skipping to next/previous song or video, "
            "seeking forward/backward, summarizing a video's content, "
            "getting video info, or showing trending videos."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action":  {"type": "STRING", "description": "play | pause | stop | resume | next | prev | seek_forward | seek_backward | seek | summarize | get_info | trending"},
                "query":   {"type": "STRING", "description": "Search query or video title to play"},
                "seconds": {"type": "INTEGER", "description": "Seconds to seek forward or backward (default: 10)"},
                "percent": {"type": "INTEGER", "description": "Percentage (0-90) to seek to in the video"},
    ...
    },
    {
        ...
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action":      {"type": "STRING", "description": "The action to perform (e.g. 'volume_set', 'brightness_set', 'volume_up', 'volume_down', 'brightness_up', 'brightness_down', 'lock_screen', 'wifi_on', 'wifi_off')"},
                "description": {"type": "STRING", "description": "Natural language description of what to do (e.g. 'set volume to 50%')"},
                "value":       {"type": "STRING", "description": "Optional value for the action (e.g. '50' for volume_set/brightness_set, 'hello' for type_text)"}
            },
            "required": []
        }
    },
...
class JarvisLive:
    ...
    def interrupt(self) -> None:
        ...
        if self._dashboard and self._loop:
            asyncio.run_coroutine_threadsafe(
                self._dashboard.broadcast({"type": "clear_audio"}),
                self._loop
            )
            ...
            if name == "open_app":
                ...
                if self._vision_busy or (_now - self._vision_last_time) < _cooldown:
                    ...
                    result = (
                        ...
                        f"Immediately say ONE natural sentence in English "
