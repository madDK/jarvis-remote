...
class DashboardServer:
    ...
    async def broadcast_audio(self, audio_bytes: bytes) -> None:
        """Broadcast raw PCM audio bytes to connected dashboard clients."""
        dead: set[WebSocket] = set()
        for ws in list(self._clients):
            try:
...
<p>Connecting to JARVIS…</p>
        ...
        @app.post("/api/screen/scroll")
        async def screen_scroll(req: Request):
            if not _auth(req):
                return JSONResponse({"error": "Unauthorized"}, status_code=401)
            try:
                import pyautogui
        ...
        @app.post("/api/screen/key")
        async def screen_key(req: Request):
            if not _auth(req):
                return JSONResponse({"error": "Unauthorized"}, status_code=401)
            try:
                import pyautogui
